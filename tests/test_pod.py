...
def test_basic(svd_algorithm):
    """Test that POD properties map correctly to the underlying SVD results."""
    n_modes = 10
    pod = POD(
        n_modes=n_modes,
    ...
    assert np.array_equal(pod.explained_energy_ratio, pod.explained_var_ratio), (
        "The POD explained energy ratio should equal "
        "the SVD explained variance ratio."
    )
    assert np.array_equal(
        pod.energy, pod.s**2
    ), "The POD energy should equal the square of the singular values."
...
def test_remove_mean():
    """Test that the mean removal produces zero-mean fluctuations."""
    X = make_dataarray("tall-and-skinny")
    n_modes = 5
    ...
    # Reconstruct fluctuations: U*S @ V = modes * s @ time_coeffs
...
def test_orthogonality():
    """Test orthogonality of POD modes."""
    X = make_dataarray("tall-and-skinny")
    n_modes = 10
    pod = POD(n_modes=n_modes)
    pod.fit(X)

    identity_k = np.eye(n_modes, dtype=np.float32)
    modes = pod.modes.data
    modes_ortho = modes.T @ modes

    assert np.allclose(
        modes_ortho, identity_k, atol=1e-5
    ), "POD modes should be orthonormal (modes.T @ modes ≈ I)."
